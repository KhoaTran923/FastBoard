...
export const BoardRepository = {
  async findByProject(projectId: string): Promise<Board[]> {
    return query<Board>('SELECT * FROM boards WHERE project_id = $1 ORDER BY position ASC', [
      projectId,
    ]);
  },

  async findById(id: string): Promise<Board | null> {
    const rows = await query<Board>('SELECT * FROM boards WHERE id = $1 LIMIT 1', [id]);
    return rows[0] ?? null;
  },
  ...
  async getColumns(boardId: string): Promise<Column[]> {
    return query<Column>('SELECT * FROM columns WHERE board_id = $1 ORDER BY position ASC', [
      boardId,
    ]);
  },
